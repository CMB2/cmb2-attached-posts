...
window.CMBAP = window.CMBAP || (function(window, document, $, undefined) {
	...
	app.cache = function() {
		var $wrap                = $( '.attached-users-wrap' );
		app.$.retrievedusers     = $wrap.find( '.retrieved' );
		app.$.attachedusers      = $wrap.find( '.attached' );
	};
	...
	app.init = function() {
		...
		// Allow the user to drag items from the left list
		app.$.retrievedusers.find( 'li' ).draggable({
			helper: 'clone',
			revert: 'invalid',
			stack: '.retrieved li',
			stop: app.replacePlusIcon,
		...
		$( '.cmb2-wrap > .cmb2-metabox' )
			// Add users when the plus icon is clicked
			.on( 'click', '.attached-users-wrap .retrieved .add-remove', app.addPostToColumn )
			// Remove users when the minus icon is clicked
			.on( 'click', '.attached-users-wrap .attached .add-remove', app.removePostFromColumn )
			// Listen for search events
			.on( 'keyup', '.attached-users-wrap input.search', app.handleSearch );
	...
	app.buildItems = function( item ) {
		...
		var $wrap  = $( item ).parents( '.attached-users-wrap' );
		// Get the ID of the item being dragged
		var itemID = item[0].attributes[0].value;
	...
	app.addPostToColumn = function() {
		...
		var $li    = $( this ).parent();
		var itemID = $li.data( 'id' );
		var $wrap  = $li.parents( '.attached-users-wrap' );
	...
	app.getPostIdsInput = function( $wrap ) {
		return $wrap.find('.attached-users-ids');
	};
	...
	app.resetItems = function( item ) {
		var $li = $( item );
		app.resetAttachedListItems( $li.parents( '.attached-users-wrap' ) );
	};

	// Replace the plus icon in the attached users column
	app.replacePlusIcon = function() {
		$( '.attached li .dashicons.dashicons-plus' ).removeClass( 'dashicons-plus' ).addClass( 'dashicons-minus' );
	};
